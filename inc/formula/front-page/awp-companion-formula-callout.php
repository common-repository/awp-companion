...
if ( $formula_cta_disabled == true ) :
	...
	if ( 'Formula' == $activate_theme || 'Formula Dark' == $activate_theme || 'Formula Light' == $activate_theme ) {
		$formula_cta_area_title  = get_theme_mod( 'formula_cta_area_title', __( 'Have any project?', 'formula' ) );
		$formula_cta_area_des    = get_theme_mod( 'formula_cta_area_des', __( 'Let"s Talk', 'formula' ) );
		$formula_cta_button_text = get_theme_mod( 'formula_cta_button_text', __( 'Contact Us', 'formula' ) );
	} else {
		$formula_cta_area_title  = get_theme_mod( 'formula_cta_area_title', __( 'Have any project?', 'formula' ) );
		$formula_cta_area_des    = get_theme_mod( 'formula_cta_area_des', __( 'Let"s Talk', 'formula' ) );
		$formula_cta_button_text = get_theme_mod( 'formula_cta_button_text', __( 'Contact Us', 'formula' ) );
	...
	if ( 'Medical Formula' == $activate_theme ) {
		$formula_cta_area_title  = get_theme_mod( 'formula_cta_area_title', __( 'Quality care, every patient, every time', 'formula' ) );
		$formula_cta_area_des    = get_theme_mod( 'formula_cta_area_des', __( 'Healthcare doesn"t have to be difficult. We"re constantly finding ways to make it easy so that you can get Better - and stay that way.', 'formula' ) );
		$formula_cta_button_text = get_theme_mod( 'formula_cta_button_text', __( 'Seek Medical Attention Now', 'formula' ) );
	}
	...
	<?php $formula_cta_background_image = get_theme_mod( 'formula_cta_background_image', awp_companion_plugin_url . 'inc/formula/img/callout/callout-bg.jpg' ); ?>
			...
			<div class="row">	
				...
				<?php if ( $formula_cta_button_text != '' ) { ?>
						<div class="m-top-40">
							<a  <?php if ( $formula_cta_button_link != '' ) { ?>
								href="<?php echo $formula_cta_button_link; ?>" 
